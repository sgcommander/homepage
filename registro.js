...
new FormCheck('frmRegistro', {
	...
	onAjaxSuccess: function(response){
		...
		switch(response){
			case '0':
				alert('Su cuenta a sido creada con éxito, puede loguearse en la parte superior para comenzar a jugar.');
...
if($defined($('btnDisponible'))){
	$('btnDisponible').addEvent('click', function(){
		if($('registroUsuario').get('value')!=''){
			...
			var req = new Request({
				url: 'index.php?controlador=Index&accion=comprobarUsuario&usuario='+$('registroUsuario').get('value')+'&servidor='+$('registroServidor').getSelected()[0].get('value')+'/index.php',
				method: 'post',
				onRequest: function(){
...
if($defined($('registroRaza'))){
	$('registroRaza').addEvent('change', function(){
		...
		if($('registroRaza').selectedIndex!=0){
			...
			var img=new Element('img',{'src':'./images/razas/'+id+'.jpg', 'alt':nombre});
